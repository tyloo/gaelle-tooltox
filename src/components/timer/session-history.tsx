...
import { Session, SessionHistoryProps } from '@/lib/types'
import { cn } from '@/lib/utils'
import { Plus, Trash2 } from 'lucide-react'
import { useState } from 'react'

export function SessionHistory({ sessions, groupedSessions, formatTime, onRemoveSession }: SessionHistoryProps) {
          ...
          <Card key={type} className='rounded-md shadow-sm'>
            ...
            >
              <div className='mb-2 flex items-center justify-between'>
                ...
                <p className='text-sm font-medium'>Total: {formatTime(group.totalDuration)}</p>
              </div>
              <div className='space-y-2'>
                {group.sessions.map((session) => (
                  <div key={session.id} className='grid grid-cols-[1fr,auto] gap-8 border py-2 px-4 text-md rounded-md'>
                    <div className='flex items-center space-y-1'>
                      ...
                      <span className='text-muted-foreground ml-1'>({formatTime(session.duration)})</span>
                    </div>
                    <div className='flex items-center justify-end gap-2'>
                      <SessionForm
                        isOpen={isDialogOpen && selectedSession?.id === session.id}
                        onOpenChange={(open) => {
                      ...
                      />
                      <Button variant='ghost' size='icon' onClick={() => onRemoveSession(session.id)}>
