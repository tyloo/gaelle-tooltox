...
function countBillableDays(start: Date, end: Date) {
  ...
  const holidays = daysInterval.filter((day) => isPublicHoliday(format(day, 'yyyy-MM-dd'))).length
          ...
          render={({ field }) => (
            <FormItem className='flex flex-col'>
              <div className='flex items-center gap-2'>
                ...
                {watchDateRange.from && watchDateRange.to && (
                  <Button type='button' variant='destructive' size='icon' onClick={resetDateRange} className='shrink-0'>
                    <X className='h-4 w-4' />
